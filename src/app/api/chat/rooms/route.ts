...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    rooms.map((room) => ({
      ...
      ticker: room.instrument?.ticker || null,
      instrumentType: room.instrument?.instrumentType || null,
      categoryId: (room.instrument as any)?.category?.id || null,
      categoryName: (room.instrument as any)?.category?.name || null,
      categorySlug: (room.instrument as any)?.category?.slug || null,
      exchangeName: (room.instrument as any)?.exchangeRel?.name || null,
      exchangeSlug: (room.instrument as any)?.exchangeRel?.slug || null,
      exchangeShort: (room.instrument as any)?.exchangeRel?.shortName || null,
