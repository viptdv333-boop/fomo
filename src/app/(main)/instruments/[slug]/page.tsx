...
import TradingViewWidget from "@/components/instruments/TradingViewWidget";
...
export default function InstrumentPage() {
  ...
  useEffect(() => {
    if (instrument) loadIdeas();
  }, [instrument?.id]);

  if (loading) {
    return <div className="text-gray-500 dark:text-gray-400 py-12 text-center">Загрузка...</div>;
  }

  if (!instrument) {
    return <div className="text-gray-500 dark:text-gray-400 py-12 text-center">Инструмент не найден</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {instrument.chatRoom && session && (
              <Link
                href={`/chat?room=${instrument.chatRoom.id}`}
      ...
      {instrument.tradingViewSymbol && (
        <TradingViewWidget symbol={instrument.tradingViewSymbol} height={450} />
      )}
